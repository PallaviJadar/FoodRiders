...
// Specialized Audio Handling to fix ERR_CACHE_OPERATION_NOT_SUPPORTED & Hostinger Path issues
app.use((req, res, next) => {
    if (req.path.startsWith('/sounds/')) {
        const soundFile = req.path.replace('/sounds/', '');
        // Search in all possible locations
        const possibleP = [
            path.join(__dirname, 'sounds', soundFile),
            path.join(__dirname, 'public', 'sounds', soundFile),
            path.join(__dirname, 'dist', 'sounds', soundFile),
            path.join(findPublicPath(), 'sounds', soundFile),
            path.join(findPublicPath(), soundFile) // case where sounds folder is flat
        ];
        
        for (let p of possibleP) {
            if (fs.existsSync(p)) {
...
function findPublicPath() {
    const paths = [
        __dirname, // 🔥 Added root as priority (for zips extracted directly to root)
    ...
    return (__dirname); // Fallback to root
};
