...
router.post('/create', async (req, res) => {
    try {
        ...
        const orderPayload = {
            ...
            // Validated amounts (never undefined)
            ...
            freeDeliveryApplied,
            paymentStatus: 'PENDING', // Rule: Default to PENDING for all at creation
        ...
        if (paymentMode.toUpperCase() === 'RAZORPAY') {
            ...
            return res.status(201).json({
                ...orderPayload,
                _id: draft._id, // Frontend uses this
                isDraft: true
            });
        }
        ...
        // ⚡ SPEED GAIN: Respond to client IMMEDIATELY
        res.status(201).json({
            success: true,
        ...
        // 📢 OFFLOAD ALL SECONDARY LOGIC (Non-blocking)
        setImmediate(async () => {
            try {
                ...
                const { emitAdminOrderAlert, emitUserNotification } = require('../socket');
