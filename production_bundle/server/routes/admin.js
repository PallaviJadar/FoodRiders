...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    // Auth Check: PIN or Password
    const secret = pin || password;
    const hash = user.pin || user.password; // Check PIN first if available, else pwd

    // Fallback: If verifying PIN but user only has password, comparison might fail if format differs?
    // Actually bcrypt handles generic strings.
    // If Admin enters PIN (1234), checks against hash.
...
router.get('/stats', auth, async (req, res) => {
  try {
    ...
    // Consolidated Stats Aggregation (Much faster than multiple counts)
    const statsData = await Order.aggregate([
      { $match: { createdAt: { $gte: resetAt } } },
      {
        $group: {
          ...
          activeOrders: { $sum: { $cond: [{ $nin: ["$status", ['DELIVERED', 'CANCELLED', 'REJECTED']] }, 1, 0] } },
