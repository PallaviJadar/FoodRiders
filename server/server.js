...
const staticFolders = ['assets', 'images', 'icons', 'banners', 'sounds', 'BgVd', 'public'];
staticFolders.forEach(folder => {
    const folderPath = path.join(baseDir, folder);
    // Remove the strict exists check at boot to allow dynamic serving
    app.use(`/${folder}`, express.static(folderPath, { maxAge: '30d', fallthrough: true }));
});

// Also serve root-level sounds if they are outside a subfolder
app.use('/sounds', express.static(path.join(baseDir, 'public', 'sounds'), { maxAge: '30d' }));
